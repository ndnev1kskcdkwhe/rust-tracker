...
`STEAM_API_KEY`, окремого ключа не треба). Так само, як з BattleMetrics у Фазі 3 — тут теж
довелось підлаштуватись під те, що API реально віддає:
...
- **Часу вайпу немає прямим полем** — витягуємо з тега `born<unix-час>` у рядку `gametype`
  (це час генерації мапи = час вайпу). Кешується на ~90 сек (`ServerCache`), бо кількість
  гравців і статус вайпу міняються набагато швидше, ніж профіль у Steam.
- **Сід мапи через Steam не передається взагалі** — жодного поля з ним немає. Тому мапа й пошук
  сервера — два окремі інструменти.

`/maps` — прев'ю мапи через RustMaps за розміром+сідом (їх треба знати заздалегідь — з конфігу
власного сервера чи зі сторінки адміна). Показує картинку з монументами і посилання на
rustmaps.com. Якщо RustMaps ще не згенерував саме цю мапу — генерація може тривати кілька
хвилин, сторінка сама допитує статус. Результат кешується назавжди (`MapCache`), бо мапа
незмінна для того самого розміру+сіда.

Якщо назва сервера в пошуку явно містить розмір і сід (наприклад `"Size: 3500 Seed: 12345"` чи
`"3500x12345"`) — картка отримує кнопку "Мапа" з прямим переходом на `/maps` (best-effort
парсинг, `src/lib/servers/parseMapSeed.ts`; спрацьовує не завжди, бо формат назви в кожного
адміна свій).
